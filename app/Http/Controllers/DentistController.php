...
class DentistController extends Controller
{
	public function __construct()
		...
		public function updatedentalrecord($id)
		{
			...
			return view('staff.dental-dentist.adddentalrecord', $params, compact('patient_info', 'patient_age', 'stacks_condition_color', 'stacks_operation_color', 'stacks_condition', 'stacks_operation', 'additional_dental_records'));
		}

		public function updatedentalrecordmodal(Request $request)
		{
			$teeth_id = $request->teeth_id;
			$teeth_info = DentalRecord::join('dental_appointments', 'dental_records.appointment_id', 'dental_appointments.id')->where('dental_appointments.patient_id', $request->patient_id)->where('teeth_id', $teeth_id)->orderBy('dental_records.created_at', 'desc')->select('condition_id', 'operation_id')->first();
		...
		{
			$params['patients'] = DentalAppointment::select('dental_appointments.patient_id', 'patient_info.patient_first_name', 'patient_info.patient_last_name')->groupBy('dental_appointments.patient_id')->join('patient_info', 'patient_info.patient_id', 'dental_appointments.patient_id')->orderBy('patient_last_name', 'asc')->paginate(20);
			$params['navbar_active'] = 'account';
		...
		{
			$params['records'] = DentalSchedule::join('dental_appointments', 'dental_appointments.dental_schedule_id', 'dental_schedules.id')->where('patient_id', $id)->whereDay('schedule_start', '<=', date('Y-m-d'))->orderBy('schedule_start', 'desc')->get();
			// dd($params['records']);
			$params['navbar_active'] = 'account';
			$params['sidebar_active'] = 'searchpatient';
