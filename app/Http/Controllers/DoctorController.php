...
class DoctorController extends Controller
{
	...
	public function confirmbillingmedical(Request $request){		
		$appointment_id = $request->appointment_id;
		$service_id = $request->checked_services_array_id;
		$service_rate = $request->checked_services_array_rate;
		// dd(sizeof($service_id));
		for($i=0; $i < sizeof($service_id); $i++){
		    $billing = new MedicalBilling;
		    $billing->medical_service_id = $service_id[$i];
		    $billing->medical_appointment_id = $appointment_id;
		    $billing->status = 'unpaid';
		    $billing->amount = $service_rate[$i];
		    $billing->save();
		}
